...
//				lobbyServer.sendRoomMessage(userSession, new SoccerGoalHandler(lobbyServer, null).getResponse(room, 3), true);
			...
			if (room.getGameMode() == GameMode.KING_SURVIVAL) {
				if (room.kingIndex == -1) {
					room.kingIndex = userSession.getUser().roomSlot;
				}
			}
...
//		output.putString(0x28, "10.0.0.50");
		...
		output.putByte(0x63, (byte) 30000);
		output.putInt(0x64, 30000);
		output.putInt(0x70, user.roomReady);
		output.putInt(0x74, user.roomCharacter);
		output.putInt(0x78, 30000); // same as unknown04 in the request
		...
		output.putInt(0xD0, 30000); // between 1 and 16
		output.putInt(0xD4, 30000); // same ^
...
//			else
		...
		output.putInt(0xF8, 30000); // this one is used in modes where you can spectate (teams). this one is compared with 0~7 (each slot)
		output.putInt(0xFC, 30000); // same ^ this one is also compared with 0~7...
...
//		}
		...
		output.putByte(0x102, (byte) 30000); // 0x59 in create room. this is master card 1.3x exp/code bonus
		output.putByte(0x103, (byte) 30000);
		...
		output.putInt(0x10C, 30000); // aura. 1 - aura. others - nothing?
		output.putInt(0x110, user.booster);
		output.putInt(0x114, 30000); // something to do with guild mark
