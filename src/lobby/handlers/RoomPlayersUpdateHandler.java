...
//		lobby.broadcastMessage(tcpServer, new RoomPlayersChangedHandler(lobby, tcpServer).getResponse(tcpServer.getUser()));
		...
		if (lobbyServer.getRoom(roomID).getGameType() == GameMode.MISSION) {
			lobbyServer.sendRoomMessage(userSession, new SpawnHandler(lobbyServer, userSession).getResponse(0), true);
		}
	}
