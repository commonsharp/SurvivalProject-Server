...
//		output.putString(0x28, "10.0.0.50");
		...
		output.putByte(0x63, (byte) 5);
		output.putInt(0x64, 5);
		output.putInt(0x70, user.roomReady);
		output.putInt(0x74, user.roomCharacter);
		output.putInt(0x78, unknown04); // same as unknown04 in the request
		...
		output.putInt(0xD0, 5); // between 1 and 16
		output.putInt(0xD4, 5); // same ^
...
//			else
		...
		output.putInt(0xF8, user.roomSlot); // this one is used in modes where you can spectate (teams). this one is compared with 0~7 (each slot)
		output.putInt(0xFC, user.roomSlot); // same ^ this one is also compared with 0~7...
...
//		}
		...
		output.putInt(0x10C, 5); // aura. 1 - aura. others - nothing?
		output.putInt(0x110, user.booster);
		output.putInt(0x114, 5); // something to do with guild mark
